---
title: Habitat Creation at Pembroke Mill Pond
seo:
  title: Habitat Creation and Ditching Pembroke
  description: Habitat creation and ditching works at Pembroke Mill Pond, including reed cutting, tree clearance and bund construction to restore water levels across a drying reed bed.
  image: ../../assets/images/after/millpond3.webp
metadata:
  title: Restoring Water to a Drying Reed Bed
  client: Private Client
  location: Pembroke, Wales
  date: 20th September, 2025
  services:
    - excavation-and-ditching
    - reed-bed-management
    - tree-work
    - trash-and-debris-removal
  sites:
    - public-private-lakes-ponds
  summary: Tree clearance, ditching and bund construction to restore water levels across a drying reed bed at Pembroke Mill Pond.
overview:
  items:
    - title: Clearing the Route
      content: |
        At Pembroke Mill Pond, a large area of reed bed was drying out, becoming increasingly boggy and losing water. The solution was to create a bund around the area to help retain water and direct it back onto the reed bed.

        The project involved a small amount of reed cutting alongside extensive tree work and clearance. Overgrown trees and hedges had to be removed along the proposed route, creating a clear path for the bund to follow.

        Once the vegetation clearance was complete, the digging arm was attached and work could begin on the ditch and bund.
      gallery: true
      media:
        type: beforeAfter
        before: ../../assets/images/before/millpond3.webp
        after: ../../assets/images/after/millpond3.webp
        beforeAlt: Before habitat creation in Mill Pond
        afterAlt: After habitat creation in Mill Pond
    - title: Ditching and Bund Construction
      content: |
        The bund was constructed along a designated route extending several hundred metres through the overgrown area.

        Material was excavated to form the ditch and build up the bund, creating a continuous barrier around the reed bed. This helped to retain water and encourage it back across areas that had previously been drying out.

        The work required careful excavation through the cleared route while maintaining the surrounding landscape and existing habitat.
      media:
        type: beforeAfter
        before: ../../assets/images/before/millpond2.webp
        after: ../../assets/images/after/millpond2.webp
        beforeAlt: Before habitat creation in Mill Pond
        afterAlt: After habitat creation in Mill Pond
    - title: Restoring the Reed Bed
      content: |
        Once the ditching and bund construction was complete, the newly formed earthworks were left to settle and naturally green over during the warmer months.

        The completed bund provided a means of retaining and directing water back onto the reed bed, helping to address the loss of water and improve conditions across the habitat.

        The project combined vegetation clearance, specialist ditching and earthworks to create a practical long-term solution for the drying reed bed.
      media:
        type: beforeAfter
        before: ../../assets/images/before/millpond1.webp
        after: ../../assets/images/after/millpond1.webp
        beforeAlt: Before habitat creation in Mill Pond
        afterAlt: After habitat creation in Mill Pond
---